...
0.2.3 普通 Chrome 回归已完成：授权页面手动采集时 6 个接口均返回，`failed_modules` 为空，当前页面 URL 未改变且没有自动切页或抢焦点。由于页面未提供可绑定酒店 ID，弹窗显示“门店需人工复核”并保留身份警告；这证明 fail-closed 状态生效，不证明该门店已具备跨酒店自动归属能力。
