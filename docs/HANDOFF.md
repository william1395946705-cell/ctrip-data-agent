...
## 2026-09-05 酒店身份绑定加固（代码完成，待浏览器重新加载后回归）
...
- 新门店人工字段核对已反馈一致，说明第二家授权门店可调用相同 6 个接口；但页面未暴露可验证酒店 ID，无法安全证明跨页面的人工确认归属。因此 0.2.3 改为 fail-closed：该类结果保持 `manual_check_required`，不写成功冷却、不提供持久化确认按钮；只有页面可见 ID 的结果才可进入已验证成功路径。真实 0.2.3 人工回归仍待浏览器重新加载扩展后完成。
- 0.2.3 代码与离线验证已完成：Python 45/45、compileall、Extension 27/27、`node --check`、MV3 打包、ZIP 完整性和 `git diff --check` 均通过。安全扫描未发现常见凭据格式；真实浏览器需重新加载扩展后再验证新版本界面状态。
