...
## 2026-09-05 酒店身份绑定加固（进行中）

- 分支 `codex/harden-hotel-identity`，基于 PR #4 合并后的 `main` 提交 `b81237de843e036c22b0bbda5b2ba3c6cd78fd94`。
- 目标：收紧扩展的页面酒店与流量接口本店行归属。新增采集前后页面身份一致性检查；页面可见酒店 ID 时必须与 flow 第一行 ID 匹配；页面未提供 ID 时保留当前授权酒店已审核行序，并把首次返回 ID 作为同名页面的本地绑定，后续变化 fail-closed。
...
- 不读取 Cookie、Profile 或 localStorage；本地仅保存现有结果中的业务酒店 ID 和同一酒店冷却/绑定键。
...
- 最终离线验证：Python 45/45、compileall、Extension 26/26、打包和 ZIP 完整性、diff 检查通过；常见凭据格式、真实测试酒店名称/ID和本机路径扫描无发现。独立复核的全部阻塞已关闭。提交、push、Draft PR 尚待执行。
