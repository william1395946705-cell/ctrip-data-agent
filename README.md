...
当前结论是：**6 个真实业务接口已在 eBooking 首页、普通订单页和订单页手工刷新后完成受控 Replay，并在同酒店、同数据日的真实页面截图对照中达到 13/13 字段一致。** 页面 URL、焦点、登录态和酒店身份保持不变。旧采集器机器输出仍因测试酒店缺少旧 Profile 映射而未完成，同时 30 天 ROAS 回退和“有违约”正样本仍未验证；因此根地图继续 discovery/disabled，扩展内置地图仍为 `unverified/disabled`。
